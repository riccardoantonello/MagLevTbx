...
#define S_FUNCTION_NAME  sfun_MagLevTbx_CurrSens
...
static void mdlCheckParameters(SimStruct *S)
{
    ...
    /* ----------------------------------------------------------------- *
     *  1st parameter check (Sensor number)
     * ----------------------------------------------------------------- */
    if( mxIsEmpty(pa0) ||
       !mxIsScalar(pa0) ||
       !mxIsNumeric(pa0) ||
       mxIsComplex(pa0) )
    {
        ssSetErrorStatus(S, "1st parameter must be a valid sensor number (0..3).");
        return;
    }
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (ssGetNumSFcnParams(S) != ssGetSFcnParamsCount(S)) 
    {
        ...
        ssSetErrorStatus(S, "Invalid number of parameters (2 parameters required: sensor id, sample time).");
        return;
    }
...
{   
    /*  get params  */
    mxUint8 sensorId = (mxUint8) mxGetScalar(ssGetSFcnParam(S, 0));
    ...
    if(!ssWriteRTWParamSettings(S, 
            ...
            SSWRITE_VALUE_DTYPE_NUM, "sensorId", (void *)&sensorId, DTINFO(SS_UINT8, 0),
